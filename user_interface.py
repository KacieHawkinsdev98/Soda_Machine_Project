...
class UserInterface:
    ...
    def display_welcome(self, continue_prompt):
    ...
    def validate_coin_choice(self, selection):
        """Translates user menu selection into the name of can that was chosen. No errors."""
        if 0 < selection <= len(self.unique_cans):
            return True, self.unique_cans[selection - 1].name
        else:
